...
impl Timeline {
            ...
            if released {
                if let Some(ptr_pos) = ptr_pos {
                    if rect.contains(ptr_pos) {
                        ...
                        let mut clip = crate::animation::Clip::new(
                            &loc.display_name,
                            frame,
                            frame + 90,
                            loc.clone(),
                        );
                        clip.color = [255, 140, 0, 100];
                        ...
                        {
                            engine.track.object_tracks[clicked_idx as usize]
                                .clips
                                .push(clip);
                        } else {
                            let track_n = engine.track.object_tracks.len() + 1;
                            let mut new_track = crate::animation::ObjectTrack::new(&format!(
                                "Location {}",
                                track_n
                            ));
    ...
    fn draw_ruler(&self, painter: &Painter, rect: Rect) {
        let frame_step = if self.zoom_x < 1.0 {
            100
    ...
    fn draw_object_clips(
        ...
        painter: &Painter,
        rect: Rect,
        track: &mut crate::animation::ObjectTrack,
        ui: &mut egui::Ui,
        current_frame: u32,
        track_idx: usize,
        selected_clip: &mut Option<(usize, usize)>,
        ...
        for (i, clip) in track.clips.iter_mut().enumerate() {
            let start_x = rect.left()
                + self.sidebar_width
                + self.pan_x
                + clip.start_frame as f32 * self.zoom_x;
            let end_x =
                rect.left() + self.sidebar_width + self.pan_x + clip.end_frame as f32 * self.zoom_x;
            ...
            let clip_rect = Rect::from_min_max(
                Pos2::new(
                    start_x.max(rect.left() + self.sidebar_width),
                    rect.top() + 4.0,
                ),
