        ...
        <div className="flex-1 flex flex-row overflow-hidden bg-background">
          ...
          <aside className="w-80 border-r border-border bg-card shrink-0 flex flex-col overflow-hidden">
            <Tabs defaultValue="media" className="flex-1 flex flex-col">
              <div className="flex items-center border-b border-border bg-[#0a0a0a]">
                <TabsList className="bg-transparent h-8 w-full justify-start rounded-none p-0 flex">
                  <TabsTrigger
                    value="media"
                    className="h-full text-[11px] font-medium gap-1.5 rounded-none border-r border-border px-4 data-[state=active]:bg-transparent data-[state=active]:text-primary text-zinc-400 transition-colors hover:bg-zinc-800/30 after:hidden shadow-none"
                  >
                    <FolderIcon size={14} weight="bold" />Media Pool
                  </TabsTrigger>
                  <TabsTrigger
                    value="effects"
                    className="h-full text-[11px] font-medium gap-1.5 rounded-none border-r border-border px-4 data-[state=active]:bg-transparent data-[state=active]:text-primary text-zinc-400 transition-colors hover:bg-zinc-800/30 after:hidden shadow-none"
              ...
              <div className="flex items-center border-b border-border bg-[#0a0a0a]">
                <TabsList className="bg-transparent h-8 w-full justify-start rounded-none p-0 flex">
                  <TabsTrigger
                    ...
                    className="h-full text-[11px] font-medium gap-1.5 rounded-none border-r border-border px-4 data-[state=active]:bg-transparent data-[state=active]:text-primary text-zinc-400 transition-colors hover:bg-zinc-800/30 after:hidden shadow-none"
                  >
                    <SlidersHorizontal size={14} weight="bold" />Inspector
                  </TabsTrigger>
                  <TabsTrigger
                    value="metadata"
                    className="h-full text-[11px] font-medium gap-1.5 rounded-none border-r border-border px-4 data-[state=active]:bg-transparent data-[state=active]:text-primary text-zinc-400 transition-colors hover:bg-zinc-800/30 after:hidden shadow-none"
                  >
                    <InfoIcon size={14} weight="bold" />Metadata
                  </TabsTrigger>
                </TabsList>
              </div>
          ...
          <div className="opacity-90 scale-100 pointer-events-none">
            {(() => {
              ...
              if (data?.type === "new-location") {
                return (
                  <div
                    className={`border rounded flex items-center shadow-2xl ${isSnapped
                ...
                return (
                  ...
                  >
                    <div className={`flex items-center gap-2 ${isSnapped ? "text-[10px] font-bold text-purple-100 uppercase tracking-tight" : "text-[11px] text-zinc-200"}`}>
                      <MagicWandIcon size={isSnapped ? 12 : 14} weight="bold" className={isSnapped ? "" : "text-primary"} />
                      {data.name}
                    </div>
                  </div>
                );
              ...
              if (data?.id && (data.type === "location" || data.type === "effect_detail")) {
                ...
                return (
                  <div
                    className={`h-10 px-3 border border-white rounded-sm flex items-center text-[10px] font-bold uppercase tracking-tight shadow-2xl ring-2 ring-white/20 ${data.type === 'location' ? 'bg-orange-800 border-orange-400 text-orange-100' : 'bg-purple-800 border-purple-400 text-purple-100'
                      }`}
                    style={{ width }}
                  >
                    {data.name}
