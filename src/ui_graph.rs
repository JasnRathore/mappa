...
impl GraphEditor {
    ...
    fn draw_curves(&self, painter: &Painter, rect: Rect, channel: &Channel) {
        if channel.keyframes.len() < 2 { return; }
        ...
        for window in channel.keyframes.windows(2) {
            ...
            match k1.interpolation {
                ...
                Interpolation::Bezier { handle_out, handle_in, .. } => {
                     ...
                     for i in 0..=steps {
                         ...
                         let t_adj = it * it * it * v_start + 3.0 * it * it * t * cp1 + 3.0 * it * t * t * cp2 + t * t * t * v_end;
                         
                         let frame = k1.frame as f32 + t as f32 * (k2.frame - k1.frame) as f32;
                         let val = k1.value.as_float() + t_adj * (k2.value.as_float() - k1.value.as_float());
                         points.push(self.frame_to_pos(frame as u32, val, rect));
                     }
                     
                     for window in points.windows(2) {
                         painter.line_segment([window[0], window[1]], Stroke::new(2.0, Color32::YELLOW));
                     }
                }
                _ => {
